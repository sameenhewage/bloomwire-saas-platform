...
# Phase 17D.1: dedicated customer WhatsApp Business App Coexistence endpoint. Admin-only + managed-mode-only;
# safe DTO with explicit connection_mode=coexistence; all Meta calls stubbed (no real HTTP/Meta). Never touches
# native /whatsapp/authorization. Fake values only. Mirrors the 17C.2 embedded-signup request spec.
RSpec.describe 'Bloomwire customer WhatsApp Coexistence Embedded Signup endpoint', type: :request do
  let(:account) { create(:account) }
  let(:admin) { create(:user, account: account, role: :administrator) }
  let(:agent) { create(:user, account: account, role: :agent) }
  let(:url) { "/api/v1/accounts/#{account.id}/bloomwire/whatsapp/coexistence_embedded_signup" }
  ...
  context 'when managed mode is active, admin, and platform ready (Meta stubbed)' do
    ...
    it 'allows an account administrator and returns a safe coexistence DTO with no secrets' do
      ...
      aggregate_failures do
        ...
        expect(response.parsed_body.dig('setup', 'connection_mode')).to eq('coexistence')
