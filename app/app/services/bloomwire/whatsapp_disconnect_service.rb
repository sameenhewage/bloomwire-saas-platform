...
#   Business Platform → Disconnect). We make NO Meta call and change NO local state; we return :mobile_action_required
#   so the caller can show the exact mobile instructions. Bloomwire reflects the change once Meta confirms it.
...
class Bloomwire::WhatsappDisconnectService
  ...
  def log_failure(error)
    Rails.logger.warn(
      "[BLOOMWIRE WA DISCONNECT] #{{ event: EVENT, operation: 'deregister', exception_class: error.class.name }.to_json}"
    )
  end
end
