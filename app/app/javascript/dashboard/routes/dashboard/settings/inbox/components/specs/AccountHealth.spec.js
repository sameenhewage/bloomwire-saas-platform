...
describe('AccountHealth — Bloomwire-managed Meta settings shortcut hiding (4.4-b-WA.2D)', () => {
  it('shows the Go to settings shortcut for a plain (non-managed) account', () => {
    isBloomwireManagedAccount.value = false;

    const html = mountHealth().html();

    expect(html).toContain('INBOX_MGMT.ACCOUNT_HEALTH.GO_TO_SETTINGS');
  });

  it('hides the Go to settings shortcut for a Bloomwire-managed account', () => {
    isBloomwireManagedAccount.value = true;

    const html = mountHealth().html();

    expect(html).not.toContain('INBOX_MGMT.ACCOUNT_HEALTH.GO_TO_SETTINGS');
  });
});
