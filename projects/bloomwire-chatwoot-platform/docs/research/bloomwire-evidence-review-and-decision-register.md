...
- **Q-04 — Secret handling** *(Both)*: Encrypt `provider_config` secrets at rest, scrub tokens/message bodies from logs, and redact Sidekiq job **displays non-mutatingly** (never dropping `text.body`/fields the worker consumes — final plan §11)? (§C.4) — informed by **S-06**.
