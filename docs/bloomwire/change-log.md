...
- **Capability (token-type-agnostic):** `Bloomwire::WhatsappMessagingCapability` now treats the token's OWN `whatsapp_business_messaging` granular scope for the WABA as the **primary** capable signal (a WABA-admin USER token verifies capable even when absent from `assigned_users`); the actor asset-task (MANAGE) path remains the fallback. A scope-read error never concludes "cannot message".
...
- **Validation (cwd `app/`, Meta stubbed, no live calls):** affected embedded/coexistence signup + capability + recheck + Meta client + setup creator + both signup request specs + messaging-capabilities request + multi-inbox E2E green; broader `spec/services/bloomwire` **340/0 (1 pending)**, `spec/requests/api/v1/accounts/bloomwire` + `spec/requests/bloomwire` **306/0**; RuboCop on all changed Ruby → **0 offenses**. New reconnect regression tests prove re-register-on-reconnect + token refresh + no duplicate.
