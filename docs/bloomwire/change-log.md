...
### Bloomwire — Atomic, dependency-safe WhatsApp Remove Inbox — IMPLEMENTED (PR #166 · implementation `52e5524` · code-complete head `49d2287` · CI 8/8)
...
- **Dependency/lifecycle fix:** `.purge!` now row-locks the account-scoped Inbox and performs attempt handling, Setup Request unlinking, Setup/history deletion, and Inbox→Channel deletion inside one database transaction. Onboarding attempts are established audit records, so terminal rows are preserved with nullable Inbox/Channel references cleared; bound active attempts are first cancelled and stripped of temporary secrets/lease/job ownership so no stale worker can repopulate the deleted aggregate. Account-level Setup Request history is likewise preserved with its optional technical-Setup link cleared. No FK, schema, migration, duplicate state, or cleanup abstraction was added.
- **Truthful async contract:** a queued Bloomwire WhatsApp removal returns HTTP **202** with safe `status=pending`; the store keeps the Inbox and the settings component does not emit completed removal/navigation while work is only accepted. Stock/non-WhatsApp and Bloomwire-OFF paths retain their existing 200/delete behavior.
- **TDD/validation:** terminal-attempt FK RED **1/1** → GREEN; atomic rollback RED **1/1** with Setup/history/attempt detachment visibly committed → GREEN; linked Setup Request RED **1/1** → GREEN; active-attempt lifecycle RED **1/1** → GREEN; API semantics RED **1/1** → GREEN; store RED **21/1** → GREEN; UI RED **11/1** → GREEN. Focused backend service/job/request **53/0**; relevant frontend **53/0**; CI-curated Bloomwire backend **1166/0 with 41 expected no-key pending**; full RuboCop **2760/0**; full ESLint **0 errors**; production/test Vite builds **45.91s / 47.72s**; migration/schema, docs parity, diff, and local secret scan clean. Exact-head CI run **29178080216** at code-complete head `49d2287` passed **8/8**: RSpec **1166/0, 41 pending** (4m39s); Vitest **386 files / 3,866 tests / 0 failures** (5m52s); RuboCop **2760/0** (1m13s); ESLint **0 errors / 371 existing warnings** (59s); Vite **1m35s**; migration/schema **57s**; docs **8s**; secret scan **7s**. This evidence-only docs stamp advances the PR head, so refreshed final-head CI and fresh exact-head review remain required.
- **Security/boundaries:** failure audits retain only internal IDs + error class and exclude exception messages, token-like values, phone numbers, raw payload/customer data. Tests mock/block provider boundaries and prove no Meta client, register/deregister, webhook subscribe/unsubscribe, onboarding processor, or provider call. No DEV/production mutation, dead-job retry, recovery, Setup recreation, credential change, merge, deploy, Enterprise change, or PR #165 modification occurred. Recovery of Inbox 50 / Channel 17 / Setup 17 remains a separate explicitly approved phase.
