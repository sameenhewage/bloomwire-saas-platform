...
- **Backend safety:** create/submit are blocked for new async work while emergency-disabled; show/poll remains available for managed queued/processing attempts; workers/recovery are switch-independent; feature OFF blocks both managed modes; account scope/admin authorization and safe DTOs remain enforced.
- **Validation (cwd `app/`; mocked/WebMock-blocked):** frontend affected Vitest **142/0**; full async backend suite **189/0 (43 pending only where local AR-encryption keys are unavailable)**; `pnpm eslint` **0 errors / 371 existing repository warnings**; RuboCop **2 files, 0 offenses**. RED evidence captured for chooser preservation, Standard async selection, persisted rollback resume, 404 semantics, Check status recovery, and emergency-disabled Restart.
