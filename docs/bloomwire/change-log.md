...
### Bloomwire — Atomic, dependency-safe WhatsApp Remove Inbox — MERGED (PR #166 · merge `b2a5f79` · base `52e5524` · persistence fence `8403cc5` · unbound identity `ba62ba7` · exact-head CI `29183074288` 8/8)
...
- **Review 4679498933 lifecycle-identity fix:** production attempts bind `phone_number_id` before local Inbox/Channel FKs, so FK-only purge discovery could miss a real active reconnect worker. Purge now preserves exact linked-audit selection and additionally selects only active attempts with the same account and one unambiguous nonblank `phone_number_id` derived from the current Channel/Setup. Missing, blank, or conflicting target identities add no match; same-WABA/different-phone and cross-account attempts remain untouched. Implementation `ba62ba7`.
- **Lock/network ordering:** persistence locks Attempt → mapping savepoint → Attempt finalization. Purge resolves stable identity without a write lock, then locks the complete exact-FK plus account/phone active-attempt set in ascending ID order → Inbox → Setup Requests/Setups → owned children. The initial opposite Inbox→Attempt order was deterministically proven to deadlock and removed. Every Meta/WhatsApp operation still completes before this DB-only persistence boundary; no transaction/row lock crosses a network call.
...
- **TDD/validation:** prior fence evidence remains. Identity RED **49/3**: purge-first recreated Channel/Inbox/Setup and completed the unbound attempt; persistence-first completed instead of blocking and locked only the bound audit; direct purge left the matching unbound worker active. GREEN identity/fence **51/0**, then concurrency **10× / 100/0**. Affected backend with test encryption **136/0, 5 expected inverse-key pending**; CI-curated backend **1181/0, 41 expected no-key pending**; full Vitest **386 files / 3,866 tests / 0 failures**; RuboCop **2761/0**; ESLint **0 errors / 371 existing warnings**; forced production Vite **41.31s**; migration/schema, diff, and local secret scan clean. Standards/Spec review found **0/0 concrete findings**. Refreshed exact-head CI and external review remain required after the docs commit.
...
### Bloomwire — WhatsApp provider migration option + truthful disconnect/removal — IMPLEMENTED; PR #165 REBASED ON MERGED #164/#166 (code head `5942a19` · final docs/push/CI/review pending · not merged/deployed)
- **Rebase/conflict resolution:** all six commits replayed onto `origin/version_1` `b2a5f79`, which contains #164 `9111cfd` and #166 `b2a5f79`. Four governance conflicts were unioned from immutable refs. #166 remains authoritative for atomic local purge, exact unbound-attempt identity, stable Attempt-before-Inbox locking, same-transaction persistence fence, HTTP 202 pending, retained store row, and no premature component navigation; #165 contributes only truthful local-only Meta copy/assertions on that surface.
- **Preservation proof:** #164 is byte-identical across all 7 merge files. #166’s authoritative service/controller/store/component/processor and backend specs are byte-identical; its sole product-spec delta retains the pending test and adds #165 copy checks. Sixteen of seventeen #165 code/spec files are byte-identical to old head `ff519db`.
- **Rebase validation:** focused backend **168/0**; frontend **6 files / 147/0**; deterministic race **10× / 100/0**; RuboCop **2763/0**; ESLint **0 errors**; forced dotenv-disabled Vite **40.30s**; docs/HTML/JSON/diff/secret/static-schema gates clean; independent Standards/Spec search reviews **0/0 findings**. Exact DB-backed schema and all eight CI jobs remain pending at the pushed final head.
- **Requirement:** offer a recommended **"Move Existing Cloud API Number"** onboarding path for a number already on the WhatsApp Cloud API with another provider, with an explicit up-front confirmation; make Coexistence disconnect and Standard disconnect **truthful**; and make Remove Inbox copy state it is local-only.
- **Change (smallest, no new architecture):** `BloomwireWhatsapp.vue` now shows three options (migration → coexistence → standard) with a recommended badge and an explicit migration-confirmation panel (Meta approval, previous-provider access loss, history not transferred, cancel keeps existing provider, no phone-number/WABA deletion). Confirmation reuses the **exact Standard lifecycle** — `coexistence:false`, `createBloomwireWhatsAppEmbeddedSignup` — via the async Standard child (new `auto-start` prop on `BloomwireWhatsappAsync.vue`, starts once only when nothing resumes) or the synchronous `register()`; **no migration endpoint, table, or Meta flag**. `Bloomwire::WhatsappDisconnectService` returns `:mobile_action_required` for Coexistence (no Meta call, no local change) and, for Standard, deregisters then **re-reads Meta status to verify DISCONNECTED** before marking local `disconnected` (else `:disconnect_unverified`, records untouched); `disconnections_controller` maps these to **409 / 502** with safe messages. `BloomwireWhatsappStatus.vue` renders the exact mobile offboarding path for Coexistence/backend-enforced `mobile_action_required` and a safe retryable failure for `disconnect_unverified`. i18n adds migration + disconnect mobile-action/failure copy and rewrites `BLOOMWIRE_REMOVE.META_NOTE` to state local-only removal with Meta/provider access possibly still active.
- **Two exact-head external reviews (reviewer @sameenhewage) applied.** Round 1 (`af76a53`) fixed the cross-inbox disconnect-state leak (`loadStatus()` resets all disconnect UI state per inbox/load generation), the acknowledgment-gated migration confirmation, and docs. **Round 2 (`27f8ce0`) rejected round-1's offboarding proof as an invalid inference** (`!coexistence_onboarded?` proves only that onboarding-readiness is absent, NOT offboarding); it is now corrected: the unsafe inverse path is removed (`WhatsappDisconnectService#recheck`/`#reconcile_coexistence`, the `POST …/disconnections/recheck` route+action, FE api/store) and replaced by **`Bloomwire::Webhooks::PartnerRemovalReconciler`** — driven by Meta's real `account_update`/`PARTNER_REMOVED` webhook in the **signed** global front door (never the message-only `Webhooks::WhatsappEventsJob`). It resolves the WABA only from authoritative payload ids (`value.waba_info.waba_id` / `entry.id`; missing/ambiguous/unknown → change nothing), marks **every coexistence setup under that WABA** `disconnected` (idempotent, records KEPT), never touches Standard, never `/deregister`, never calls Meta, logs a sanitized summary (masked WABA tail). The mobile-panel button is now a pure persisted-status **Refresh** (reads `setup.status`, which only the webhook can flip). **Migration copy corrected:** the payment prerequisite now names the **SOURCE (existing) WABA** (not the destination); partner/destination **credit-line sharing is conditional ("when applicable")**; availability is **environment-independent** ("Bloomwire-assisted; manager-confirmed") — the stale "Development mode / Standard access" text is removed (two-step-off-first, verified business/WABA, manager-vs-Bloomwire split, acknowledgment gate retained).
- **Exact-head review `4678508407` of `1f21c49` — 2 remaining blockers fixed in `6a0e715`:** (1) **mixed/batched webhook loss** — Meta officially documents signed POST updates aggregated in batches up to 1000; the previous whole-request early return dropped message/status siblings. The signed global front door now splits every `entry[].changes[]` item, handoff-checks and enqueues each non-`account_update` single-change payload exactly once **before** reconciliation, and passes the reconciler one payload containing only authoritative `field=account_update && event=PARTNER_REMOVED` changes. Unrelated account updates reach neither owner; invalid signatures reach neither; native route/job unchanged. (2) **Standard false success without Setup** — before either mode or any Meta-client construction, disconnect now requires the preserved Setup to match current account + requested inbox + WhatsApp channel + channel id + channel `phone_number_id`; missing/misaligned/stale → safe `not_found`, zero Meta calls, no success DTO/record change. Valid Standard and Coexistence preserve the same IDs/counts and prior contracts. Stale disconnect-spec headers corrected. Direct Meta fetches returned **HTTP 429**, but official browser-readable `.md` pages supplied the documented batching/signature/`PARTNER_REMOVED`/Coexistence-no-`deregister` evidence; no contract invented.
- **Validation (round 3, implementation-agent local):** mixed-batch RED **1/1** (`0` message handoffs) → GREEN; missing-Setup RED **1/1** (Meta client constructed) → GREEN. Final front door **19/0**; disconnect service **12/0**; request **8/0**; relevant backend **107/0**; final broad backend **1188/0, 41 expected pending** (local AR-encryption keys absent); relevant FE **192/0**; RuboCop **5/0**; ESLint **0 errors / 371 existing warnings**; Vite **46.05s**; docs governance, migration/schema, diff, and secret scan clean. Cumulative PR **21 files** (17 code/spec + 4 docs, **+1952/−101**). Exact-head CI **run `29164573312` at `1506c6d`: 8/8 green** (RSpec, Vitest, RuboCop, ESLint, Vite, migration/schema, docs, secret scan). This CI-evidence docs stamp necessarily advances head; refreshed final-head CI is recorded in the PR body/GitHub. This is self-validation, **not independent QA**; fresh exact-head GPT review required.
- **Prior round-2 validation:** reconciler RED **11** → GREEN **11/0** (valid / multiple-under-one-WABA / standard-untouched / unrelated-event / ambiguous / missing / unknown / idempotent / records-preserved / malformed); front-door `account_update` **14/0** (reconciles + no message-job enqueue + signature rejection + safe redacted logs + idempotency); FE refresh RED → GREEN focused **102/0**; migration copy-fix asserts GREEN. Broader FE **192/0**; broader Bloomwire BE **828/0** (41 pending, pre-existing AR-encryption env gate). RuboCop clean; ESLint clean; Vite build OK; docs governance OK; no schema/dependency change; no secrets. Cumulative PR diff **21 files** (17 code/spec **+1587/−68**, 4 docs). Full CI re-run on push. Meta docs still unreachable (HTTP 429) — implemented against the known `account_update`/`PARTNER_REMOVED` schema, fail-closed against field-name variance. The same-agent Standards/Spec pass is a self-review, **not** the repo's independent QA gate; a fresh exact-head review is requested.
- **Boundaries/security:** no live Meta/WhatsApp call (all mocked/WebMock-blocked); no real DB, register/deregister, reconnect, schema/table/dependency/route/Enterprise/native-webhook/production change; feature-OFF remains 404/inert; safe failure DTOs contain no token/phone/WABA and logs stay redacted. Inbox 50 / Channel 17 / Setup 17 were not touched. **Real-Meta migration plus observed Coexistence `PARTNER_REMOVED`, DEV runtime proof, and independent QA are NOT run** — DEV app remains Development mode / Standard access. Not merged or deployed; **merge SHA + DEV PASS pending**.
