...
### Bloomwire — Coexistence disconnected reconnect must use Meta's official onboarding readiness — LOCAL QA PASS; PR/deploy/runtime pending
- **Requirement/runtime:** no second Standard attempt. Reconnect the preserved WhatsApp Business App number through Coexistence and reuse 50/17/17. Deployed `4bbeecda` preserves one disconnected binding and zero active Standard attempts, but Channel 17 remains marked Standard; a safe live GET returned HTTP 200 with `status=DISCONNECTED`, `is_on_biz_app=false`, `platform_type=CLOUD_API`. Coexistence skips `/register`, finds no connected duplicate, and returns `no_connected_registration`.
- **Root cause:** the Coexistence popup owns the provider transition. Meta documents the post-popup readiness pair as `is_on_biz_app=true` plus `platform_type=CLOUD_API`; Bloomwire exposed only Standard `status=CONNECTED`. Shared reconnect persistence reused records correctly but never applied the Coexistence mode override to an existing channel.
- **Fix:** `Whatsapp::FacebookApiClient` reads only the two official fields through the existing timeout/sanitization helper. `WhatsappCoexistenceEmbeddedSignupService` uses that exact pair only after a non-CONNECTED status, still never calls `/register`, then follows existing capability/subscription/persistence. A no-op-by-default reconnect hook runs inside the existing mapping transaction; the Coexistence override marks the reused channel `source=bloomwire_managed` and `connection_mode=coexistence` without replacing other provider config.
- **TDD/validation:** public-service RED **1/1** → GREEN **1/0**; Coexistence **18/0**; Graph client **34/0**; Coexistence request + Standard inverse **64/0**; affected matrix **170/0 with 25 expected no-key pending**; keyed async processor consumer **25/0**. Automated-test provider calls mocked/WebMock-blocked; one read-only baseline GET occurred; no provider mutation. RuboCop **5/0**; Standards/Spec review **0/0 findings**; diff/docs/HTML/secret checks clean.
- **Boundaries/residual:** no Standard fallback, `/register`/`deregister`, frontend/API/schema/router/Enterprise/production change, duplicate record, blind credential rotation, or live attempt. 50/17/17 remain untouched on DEV. PR, merge, exact-SHA deploy, then explicit approval for exactly one Coexistence popup remain pending. Interim verdict: **DEPLOYED — COEXISTENCE RECONNECT FIX STILL REQUIRED**.
