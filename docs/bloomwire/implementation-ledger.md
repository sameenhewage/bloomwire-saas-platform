...
- **Capability:** `Bloomwire::WhatsappMessagingCapability` treats the token's own `whatsapp_business_messaging` granular scope for the WABA as the primary capable signal (token-type-agnostic — a WABA-admin USER token verifies without appearing in `assigned_users`); the MANAGE asset-task path is the fallback; a scope-read error never concludes incapable.
...
- **Validation (cwd `app/`, Meta stubbed):** affected embedded/coexistence signup + capability + recheck + Meta client + setup-creator + both request specs + messaging-capabilities request + multi-inbox E2E green, incl. new reconnect regression tests (re-register-on-reconnect · token refresh · no duplicate); `spec/services/bloomwire` **340/0 (1 pending)**; `spec/requests/api/v1/accounts/bloomwire`+`spec/requests/bloomwire` **306/0**; RuboCop on changed Ruby → **0 offenses**. No secrets/DTO leak; no live Meta calls (WebMock).
