...
| WhatsApp provider migration + truthful disconnect/removal | Recommended "Move Existing Cloud API Number" option (explicit confirm, reuses Standard lifecycle `coexistence:false`); Coexistence disconnect → `mobile_action_required`; Standard disconnect verified before local change; local-only Remove Inbox copy | _pending_ | Implemented · FE 107/107 · BE 15/15 · lint/build clean · PR pending · not merged/deployed · real-Meta cert deferred |
...
### WhatsApp provider migration + truthful disconnect/removal — `Implemented · local full pass · PR pending · not merged/deployed`
...
- **Validation:** FE RED **12** → GREEN; focused FE **107/107**; broader FE **161/161**. BE RED **7** → GREEN; focused BE **15/15**; broader Bloomwire BE **425 examples, 0 failures, 27 pending** (pre-existing AR-encryption-key env gate). RuboCop **4/0**; ESLint clean; Vite build **35.22s**; i18n JSON valid; secret scan clean; diff **12 files (+711/−65)**.
- **Boundaries/security:** no live Meta/WhatsApp call (all mocked); no schema/table/dependency/Enterprise/production change; feature-OFF preserved (disconnect endpoint 404 when Bloomwire OFF; Vue changes only inside already-gated components); safe DTOs only. **Real-Meta migration/disconnect certification not run** — DEV app is in Development mode / Standard access; deferred and flagged as a blocker.
