...
| Atomic WhatsApp Remove Inbox | Transactional dependency ordering, attempt/Setup Request audit detachment, safe retries, truthful 202 pending UI | #166 | Implemented `52e5524` · code-complete head `49d2287` CI 8/8 · refreshed final-head CI/review pending · DEV recovery not performed |
...
### Atomic, dependency-safe WhatsApp Remove Inbox — `Implemented · PR #166 · implementation 52e5524 · code-complete 49d2287 CI 8/8 · not deployed`
...
- **Dependency/lifecycle ownership:** the target Inbox is account-scoped and row-locked. One database transaction now owns attempt handling → Setup Request unlink → Setup/history deletion → Inbox/Channel destruction. Onboarding attempts remain the ADR-0010 audit source: terminal attempts are detached via their nullable Inbox/Channel FKs; bound active attempts first become secret-free `cancelled` with lease/job ownership cleared, then detach, preventing stale workers from recreating a permanently removed aggregate. Account-level Setup Request intake history is preserved and unlinked from its optional technical Setup. No FK weakening or schema migration.
- **Async/API/UI truth:** accepted Bloomwire WhatsApp removal returns safe HTTP **202** + `status=pending`. The Inbox store does not delete the record and the component does not emit `removed` while completion is unproven. Stock/non-WhatsApp and master-OFF responses remain unchanged.
- **TDD/validation:** RED terminal FK **1/1**; rollback **1/1**; Setup Request FK **1/1**; active-attempt lifecycle **1/1**; API 200-vs-202 **1/1**; store **21/1**; UI **11/1**. GREEN focused backend **53/0**; relevant frontend **53/0**; CI-curated Bloomwire backend **1166/0 with 41 expected no-key pending**; full RuboCop **2760/0**; full ESLint **0 errors**; production/test Vite **45.91s / 47.72s**; migration/schema, docs parity, diff, and secret gates clean. Exact-head CI run **29178080216** at code-complete `49d2287` passed **8/8**: RSpec **1166/0, 41 pending**; Vitest **386 files / 3,866 tests / 0 failures**; RuboCop **2760/0**; ESLint **0 errors / 371 existing warnings**; Vite, migration/schema, docs governance, and secret scan passed. This evidence-only docs stamp advances the head; refreshed final-head CI and fresh exact-head review remain required.
- **Security/scope:** local failure audits log only internal IDs/error class and exclude exception message, token-like value, phone, raw payload, and customer data. Provider boundaries are mocked/WebMock-blocked; no Meta, register/deregister, webhook subscribe/unsubscribe, onboarding processor, or provider call. No Standard/Coexistence disconnect, PR #165, Enterprise, production, dependency, schema, or deployed runtime change.
