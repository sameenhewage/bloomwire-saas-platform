...
- **Validation:** corrective RED→GREEN: switch flip **12/1 → 12/0**; stale job **7/1 → 7/0**; replay/status **15/2 → 15/0**. Final encryption-enabled matrix **201/0 (5 expected no-key pending)**; no-key fail-closed **49/0 (14 expected encrypted-only pending)**; shared regressions **72/0**; affected frontend **8 files / 144 tests / 0 failures**; full RuboCop **2759/0**; ESLint **0 errors / 371 existing warnings**; i18n, docs governance/parity, diff check, and secret scan (**64 files / 0 hits**) passed. Isolated upgrade baseline→apply→rollback→reapply passed with **29 columns / 7 indexes / 3 FKs / 2 checks** and zero audit-DB residue. Legacy clean-install incompatibility is tracked separately as non-blocking **#151**. No live Meta/WhatsApp calls; no deploy/DEV/production change.
