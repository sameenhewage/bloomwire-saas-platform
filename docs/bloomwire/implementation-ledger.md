...
| Atomic WhatsApp Remove Inbox | Transactional dependency ordering + DB persistence fence, attempt/Setup Request audit detachment, safe retries, truthful 202 pending UI | #166 | Base `52e5524` · review `4679360111` fence `8403cc5` · code CI `29181043448` 8/8 · final-head CI/review pending · DEV recovery not performed |
...
### Atomic, dependency-safe WhatsApp Remove Inbox — `Implemented · PR #166 · base 52e5524 · review 4679360111 fence 8403cc5 · not deployed`
...
- **Dependency/lifecycle ownership:** one transaction owns attempt handling → Setup Request unlink → Setup/history deletion → Inbox/Channel destruction. Terminal attempts remain detached audit records; bound active attempts become secret-free `cancelled`. Review `4679360111` proved cancellation alone was insufficient after a worker had renewed: its separate persister transaction could recreate the aggregate before a stale Attempt write failed. The root fix validates active status + owner + generation + valid lease under the Attempt row lock that encloses mapping persistence and Attempt finalization; local processor writes use the same fence. No FK/schema migration.
- **Lock/network contract:** async persistence locks Attempt → mapping savepoint → Attempt finalization. Purge locks related Attempts by ascending ID → Inbox → Setup Request/Setup → owned children. Deterministic testing exposed and removed the opposite-order deadlock. Meta/network stages remain outside every DB transaction/row lock; the fenced section is DB-only.
...
- **TDD/validation:** original RED/GREEN remains. Race RED **10/10** proved aggregate recreation, absent purge wait, four stale predicate failures, three unguarded stale writes, and mapping rollback leakage. GREEN fence **10/0**, repeated **10× / 100/0**. Affected backend **163/0**; CI-curated local backend **1181/0, 5 expected inverse-key pending**; focused frontend **32/0**; full Vitest **386 files / 3,866 tests / 0 failures**; RuboCop **2761/0**; ESLint **0 errors / 371 existing warnings**; production Vite **32.29s**; schema/diff clean; Standards/Spec self-review **0/0 concrete findings**. Code CI run **29181043448** at `8403cc5` passed **8/8**: RSpec **1176/0, 41 pending**; Vitest **386 files / 3,866 tests / 0 failures**; RuboCop, ESLint, Vite, migration/schema, docs, and secret scan passed. Refreshed final-head CI and external exact-head review remain required.
