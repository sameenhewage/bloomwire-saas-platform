...
| Managed WhatsApp honest progress | Replace unreachable generic Add Agents/Finish sidebar steps with the real `Choose Channel → Connect WhatsApp` flow; success card owns Done | _pending_ | Implemented · review/CI/DEV pending |
...
### Managed WhatsApp honest progress — `Implemented · PR/merge/DEV pending`
...
- **TDD/validation:** visible contract RED **1/1** (four generic titles) → GREEN **3/0** (managed two-step, OFF inverse, non-WhatsApp inverse); focused adjacent matrix **38/0**; full frontend **386 files / 3,864 tests / 0 failures**; full ESLint **0 errors / 371 existing warnings**; production Vite build **39.02s**; English translation JSON, docs governance, HTML parse/markers, and diff checks clean. Independent QA/review, CI, and DEV browser proof remain pending.
- **Boundaries/residual:** frontend/i18n/spec/docs only. No shared `Wizard` behavior, router, backend, DB/schema, inbox-agent assignment, onboarding lifecycle, provider state/credential, Meta call, Enterprise code, production, or chat source-of-truth change; no dependency or secret. Mobile still omits the desktop wizard as before.
