...
| Atomic WhatsApp Remove Inbox | Transactional dependency ordering + DB persistence fence + exact unbound-attempt identity, audit detachment, safe retries, truthful 202 pending UI | #166 | Merged `b2a5f79` · base `52e5524` · fence `8403cc5` · review `4679498933` fix `ba62ba7` · exact-head CI `29183074288` 8/8 · DEV recovery not performed |
| WhatsApp provider migration + truthful disconnect/removal | Recommended "Move Existing Cloud API Number" option (explicit confirm + prerequisites/ack gate, reuses Standard lifecycle `coexistence:false`); signed front door splits batched changes exactly once and sends only `account_update/PARTNER_REMOVED` to the reconciler; persisted-status **Refresh**; Standard disconnect requires an account/inbox/channel/phone-aligned preserved Setup before Meta and verifies DISCONNECTED; local-only Remove Inbox copy | #165 | Rebased code head `5942a19` onto merged #164 `9111cfd` + #166 `b2a5f79` · focused BE 168/0 · FE 147/0 · race 100/0 · RuboCop 2763/0 · ESLint/Vite/docs/secret/static-schema clean · Standards/Spec 0/0 · final push/8-job CI/review pending · not merged/deployed |
...
### Atomic, dependency-safe WhatsApp Remove Inbox — `Merged · PR #166 · b2a5f79 · exact-head CI 29183074288 8/8 · DEV recovery not performed`
- **Product/root-cause truth:** one owner-issued DEV deletion entered the existing async deprovision, removed Setup 17 and Inbox 50 history, then failed at `inbox.destroy!` because five retained onboarding-attempt audit rows still referenced Inbox 50 through restrictive FK `fk_rails_9ac8ff5f9a`. Three retries repeated the FK failure and the job dead-lettered. Without an outer transaction, earlier deletes remained committed.
- **Current DEV state / recovery boundary:** Inbox 50 and Channel 17 remain; Setup/binding count is 0; five terminal attempts remain; messages/conversations/contact-inboxes/reporting events are 0; the encrypted Channel credential is present. No dead-job retry, deletion completion, Setup recreation, record restore, credential change, reconnect, provider mutation, or other recovery has occurred. Recovery is deliberately separate from this code PR.
- **Dependency/lifecycle ownership:** one transaction owns attempt handling → Setup Request unlink → Setup/history deletion → Inbox/Channel destruction. Terminal attempts remain detached audit records; active attempts become secret-free `cancelled`. Review `4679360111` added the persistence fence. Review `4679498933` proved real attempts are unbound locally until finalization: purge now combines exact linked audits with active attempts matching the same account and one unambiguous nonblank current Channel/Setup `phone_number_id`. Blank/missing/conflicting identity adds no match; different-phone and cross-account attempts remain untouched. No FK/schema migration.
- **Lock/network contract:** async persistence locks Attempt → mapping savepoint → Attempt finalization. Purge resolves identity, then locks the complete exact-FK plus account/phone active set by ascending ID → Inbox → Setup Request/Setup → owned children. Production-shaped persistence-first testing proves purge blocks on the unbound Attempt, then removes both bound and newly bound audit rows in stable order without deadlock. Meta/network stages remain outside every DB transaction/row lock.
- **Async/API/UI truth:** accepted Bloomwire WhatsApp removal returns safe HTTP **202** + `status=pending`. The Inbox store does not delete the record and the component does not emit `removed` while completion is unproven. Automatic completion reconciliation is deferred; refresh reloads the authoritative Inbox list. Stock/non-WhatsApp and master-OFF responses remain unchanged.
- **TDD/validation:** prior fence evidence remains. Identity RED **49/3** proved purge-first resurrection, missing persistence-first wait/lock, and direct failure to cancel an exact unbound worker. GREEN **51/0**, repeated race **10× / 100/0**. Affected backend **136/0, 5 expected inverse-key pending**; CI-curated backend **1181/0, 41 expected no-key pending**; full Vitest **386 files / 3,866 tests / 0 failures**; RuboCop **2761/0**; ESLint **0 errors / 371 existing warnings**; forced production Vite **41.31s**; schema/diff/secret gates clean; Standards/Spec review **0/0 concrete findings**. Refreshed exact-head CI and external review remain required.
- **Security/scope:** failure audits remain internal-ID/error-class only; provider boundaries are mocked/WebMock-blocked. Official Meta overview HTML and `.md` fetches failed with TLS handshake timeouts; no 429/login response or provider call occurred. No Standard/Coexistence disconnect, PR #164/#165, Enterprise, production, dependency, schema, deployed runtime, recovery, or credential change.

### WhatsApp provider migration + truthful disconnect/removal — `Implemented · PR #165 rebased code head 5942a19 onto b2a5f79 · final push/CI/review pending · not merged/deployed`
- **Rebase integration:** six commits replayed onto the base containing #164/#166. Four docs conflicts preserve both histories. #164’s 7 merge files and #166’s authoritative service/controller/store/component/processor/backend specs are byte-identical to the new base. The sole #166 product-spec delta adds #165 local-only Meta-copy assertions and retains #166’s pending-without-emit contract; 16/17 #165 code/spec files remain byte-identical to old `ff519db`.
- **Current validation:** focused backend **168/0**; focused frontend **6 files / 147/0**; deterministic race **10× / 100/0**; RuboCop **2763/0**; ESLint **0 errors**; forced dotenv-disabled Vite **40.30s**; docs/HTML/JSON/diff/secret/static-schema checks clean; independent Standards/Spec search reviews **0/0 concrete findings**. GitHub’s DB-backed schema and all eight exact-head jobs remain pending.
- **Two exact-head external reviews (reviewer @sameenhewage) applied.** **Round 1 (`af76a53`)** fixed the cross-inbox disconnect-state leak (`BloomwireWhatsappStatus.vue#loadStatus()` resets all disconnect UI state per inbox/load generation; regressions Coexistence-A→Standard-B and →non-managed), the acknowledgment-gated migration confirmation, and docs. **Round 2 (`27f8ce0`) rejected round-1's offboarding proof as an invalid inference** — `!coexistence_onboarded?` proves only that the onboarding-ready pair (`is_on_biz_app=true && platform_type=CLOUD_API`) is **absent**, not that the number was offboarded. Corrected: the unsafe inverse path is removed (`WhatsappDisconnectService#recheck`/`#reconcile_coexistence`, the `POST …/disconnections/recheck` route+action, FE api/store) and replaced by the authoritative **`Bloomwire::Webhooks::PartnerRemovalReconciler`**, driven by Meta's real `account_update` webhook with `event=PARTNER_REMOVED` in the **signed** global front door (`Bloomwire::Webhooks::WhatsappController#process_payload` branches to it and never enqueues the message-only `Webhooks::WhatsappEventsJob`). It resolves the WABA only from authoritative payload ids (`value.waba_info.waba_id` preferred, else `entry.id`; missing → `:missing_waba`, disagree → `:ambiguous_waba`, no mapping → `:unknown_waba` — all change nothing), marks **every coexistence setup under that WABA** `disconnected` (idempotent; non-disconnected rows only), **KEEPS** all records, never touches Standard, never `/deregister`, never calls Meta, logs a sanitized summary (masked WABA tail). The mobile-panel button is now a pure persisted-status **Refresh** (`fetchBloomwireWhatsAppCapability` → flips only when persisted `setup.status` is `disconnected`). **Migration copy corrected:** the payment prerequisite names the **SOURCE (existing) WABA** (not the destination); partner/destination **credit-line sharing is conditional ("when applicable")**; availability is **environment-independent** ("Bloomwire-assisted; manager-confirmed") — the stale "Development mode / Standard access" text is removed. Meta docs still unreachable (HTTP 429) — implemented against the known `account_update`/`PARTNER_REMOVED` schema, fail-closed against field-name variance. The same-agent Standards/Spec pass is a self-review, **not** the repo's independent QA gate.
- **Third exact-head review (`4678508407` of `1f21c49`) fixed in `6a0e715`:** (1) **batch correctness** — official Meta docs say HMAC-validated POST updates are aggregated up to 1000; the previous `return reconcile_account_update(payload) if account_update?(payload)` lost message/status siblings. The front door now splits all `entry[].changes[]`; each non-account-update single-change payload is handoff-checked/enqueued exactly once **before** reconciliation, and one filtered reconciler payload contains only `field=account_update && event=PARTNER_REMOVED`. Unrelated account updates reach neither owner; invalid signatures reach neither; native route/job unchanged. (2) **disconnect mapping correctness** — disconnect requires exact current-account + requested-inbox + WhatsApp-channel + channel-id + channel-phone-id Setup alignment before either mode or Meta-client construction. Missing/wrong/stale mapping returns safe `not_found`, zero Meta calls, no success DTO/record change; valid Standard/Coexistence retain existing IDs/counts/contracts. Direct URL reads were HTTP 429, but official browser-readable `.md` pages provided batching/signature/`PARTNER_REMOVED`/no-Coexistence-`deregister` evidence. No undocumented contract or live call. Stale spec headers corrected.
- **Product truth:** a customer whose number is already on the WhatsApp Cloud API with another provider needs a clear, recommended way to move it to Bloomwire, and must understand the consequences before anything happens. Disconnect and Remove Inbox must be honest about what actually changes.
- **Onboarding:** `BloomwireWhatsapp.vue` now opens on three options in order — **Move Existing Cloud API Number (migration, recommended)** → Connect Existing WhatsApp Business App (Coexistence) → Register New Number (Standard). Migration shows an explicit confirmation (Meta approval, previous-provider messaging-access loss, history not transferred, cancel/reject keeps the existing provider, no phone-number/WABA deletion). Confirmation reuses the **exact Standard lifecycle** with `coexistence:false`: when async Standard is enabled it hands off to `BloomwireWhatsappAsync.vue` (new `auto-start` prop → starts once, never alongside a resumed attempt), otherwise it runs the synchronous `register()` and posts to `createBloomwireWhatsAppEmbeddedSignup`. **No migration endpoint, table, or Meta flag** was added.
- **Disconnect ownership (truthful):** `Bloomwire::WhatsappDisconnectService` now branches on `provider_config['connection_mode']`. Coexistence → `:mobile_action_required` with **no Meta call and no local change** (offboarding is a mobile action in the WhatsApp Business app). Standard → deregister then **re-read the authoritative Meta status and only mark `disconnected` on verified `DISCONNECTED`**; a missing-cred / raise / still-CONNECTED path returns `:disconnect_unverified` and leaves every record untouched. `disconnections_controller` maps `mobile_action_required` → **409** and `disconnect_unverified` → **502** with safe, non-secret messages. `BloomwireWhatsappStatus.vue` renders the exact mobile offboarding path (Coexistence click or backend-enforced `mobile_action_required`) and a safe retryable failure for `disconnect_unverified`; it never self-declares `disconnected`.
- **Remove Inbox truthfulness:** `BLOOMWIRE_REMOVE.META_NOTE` now states the removal is local-only, makes no Meta call, and that the number can remain registered/connected at Meta with previous provider access possibly still active — no WABA/phone deletion. Copy-only in this slice; the deprovision service is unchanged.
- **Validation (round 3, implementation-agent local):** mixed-batch RED **1/1** (`enqueued_payloads=0`) → GREEN; missing Setup RED **1/1** (`FacebookApiClient.new` called) → GREEN. Final front door **19/0**; disconnect service **12/0**; request **8/0**; relevant backend **107/0**; final broad backend **1188/0, 41 expected pending** (AR encryption keys absent); relevant FE **192/0**; RuboCop **5/0**; ESLint **0 errors / 371 existing warnings**; Vite **46.05s**; docs governance, migration/schema, diff, and 21-file secret scan clean. Cumulative PR **21 files** (17 code/spec + 4 docs, **+1952/−101**). Exact-head CI **run `29164573312` at `1506c6d`: 8/8 green** (RSpec, Vitest, RuboCop, ESLint, Vite, migration/schema, docs, secret scan). This CI-evidence docs stamp advances head; refreshed final-head CI lives in the PR body/GitHub. **Self-validation only — not independent QA.**
- **Boundaries/security:** no live Meta/WhatsApp call (all mocked/WebMock-blocked); no real DB/register/deregister/reconnect/schema/table/dependency/route/Enterprise/native-webhook/production change; feature-OFF stays 404/inert; safe DTO/redacted logs only; Inbox 50 / Channel 17 / Setup 17 untouched. **Independent QA, full CI, DEV runtime, real-Meta migration, and an observed Coexistence `PARTNER_REMOVED` are pending.** PR open; not merged/deployed; merge SHA + DEV PASS pending; fresh exact-head GPT review required.
