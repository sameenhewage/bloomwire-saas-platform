...
| ADR-0010 Coexistence reconnect readiness | Official `is_on_biz_app` + `platform_type` readiness and transactional mode correction for reused records | _pending_ | Local QA PASS on `fix/bloomwire-coexistence-disconnected-reconnect`; PR/deploy/runtime pending |
...
### ADR-0010 Coexistence disconnected reconnect readiness — `Local QA PASS · PR/deploy/runtime pending`
- **Product/deployed truth:** the owner requires one future Coexistence reconnect of the same WhatsApp Business App number, never another Standard attempt. Exact deployed `4bbeecda` keeps one 50/17/17 binding, Setup/Meta DISCONNECTED, Channel 17 marked Standard, and 0 active attempts; a safe live GET returned HTTP 200 with `status=DISCONNECTED`, `is_on_biz_app=false`, `platform_type=CLOUD_API`. The dedicated Coexistence flow skips `/register`, but a still-DISCONNECTED status falls into the connected-duplicate resolver and returns `no_connected_registration`.
- **Root owner:** Meta Business App Embedded Signup performs the supported provider transition. Its official optional verification is `is_on_biz_app=true` plus `platform_type=CLOUD_API`; Standard `status=CONNECTED` is not the sole Coexistence readiness contract. Bloomwire lacked this query. Reconnect persistence also bypassed the Coexistence create override, so reused channels retained old mode metadata.
- **Smallest fix:** add the two-field Graph read behind existing per-call timeouts; Coexistence alone accepts the exact official pair after a non-CONNECTED status and still skips Standard `/register`. Add one default-no-op persistence hook inside the existing transaction; Coexistence uses it to merge its source/mode onto the reused channel before the existing mapping write. Same records and safe DTO remain authoritative.
- **TDD/validation:** public service RED **1/1** → GREEN **1/0**; Coexistence **18/0**; client **34/0**; Coexistence request + Standard inverse **64/0**; affected **170/0 with 25 expected no-key pending**; keyed processor/persistence consumer **25/0**. Automated tests made no provider calls; one read-only baseline GET occurred with no provider mutation. RuboCop **5/0**; Standards/Spec review **0/0 findings**; diff/docs/HTML/secret checks clean.
- **Boundaries/residual:** no Standard fallback or attempt, no manual `/register`/`deregister`, no blind credential rotation, no frontend/controller/route/schema/router/Enterprise/production change, and no live 50/17/17 mutation. PR/merge/deploy pending; only afterward request approval for exactly one Coexistence popup. Interim verdict: **DEPLOYED — COEXISTENCE RECONNECT FIX STILL REQUIRED**.
