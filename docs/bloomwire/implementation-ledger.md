...
| ADR-0010 disconnected reconnect hotfix | Finalized crash-resume excludes preserved `disconnected` setups so fresh OAuth runs exchange/register/persist | _pending_ | Implementation complete on `fix/bloomwire-whatsapp-disconnected-reconnect-fast-path`; PR/redeploy/provider certification pending |
...
### ADR-0010 disconnected reconnect fast-path correction — `Implementation complete · PR/redeploy pending`
...
- **DEV outcome/boundaries:** the single submitted code expired before same-attempt recovery and was safely cleared as terminal `expired`; Meta/Setup remained DISCONNECTED; 50/17/17 and one binding preserved; queues 0/0/49; no second attempt. No schema/frontend/Coexistence/router/Enterprise/production change. The emitted root token-shaped line was verified invalid; DEV/Meta browser sessions and storage were invalidated/cleared. Hotfix PR, CI, exact-head review, merge, redeploy, and a separately approved provider retry remain pending.
...
- **Runtime/residual:** PR #155 head `f37467c`, CI 8/8, merge `a709528`, DEV run `29091137113`; exact SHA/health/migrations/cron/volumes verified. Waiting/relaunch/submit lifecycle passed on one live Standard attempt, but provider reconnect was blocked by the separately documented disconnected fast-path bug. Hotfix/redeploy and any separately approved provider/messaging certification remain pending.
