...
| Managed Account Health webhook ownership | Expect the global callback for `source=bloomwire_managed`; preserve native per-phone callback semantics | _pending_ | Implemented · review/CI/DEV pending |
...
### Managed Account Health webhook ownership — `Implemented · PR/merge/DEV pending`
