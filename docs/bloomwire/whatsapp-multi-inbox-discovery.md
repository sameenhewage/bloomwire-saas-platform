...
Status: **PR #112 — open, non-draft, ready for review (NOT merged).** Docs-only discovery + ADR — no code, no route, no frontend, no migration, no production, no real Meta/WhatsApp calls. Locks the Bloomwire **multiple WhatsApp inbox per account** business model and its guardrails **before** the 17E hardening slices. (The PR head SHA advances with each docs commit and is intentionally not pinned here — see GitHub for the current head.)
